...
import { IMoexApiResponseCurrentPrices } from '../moex/types';
...
export class MoexPositions {
  ...
  async calculateBondsPositions(): Promise<void> {
    ...
    const bondTickersString: string = this.bondDeals
      .map(d => d.ticker)
      .join(',');
    const bondPrices: IMoexApiResponseCurrentPrices =
      await this.moexApi.getStocksCurrentPrices(
        MoexMarket.bonds,
        bondTickersString,
      );
    ...
    moexBondModels.forEach(bond => {
      ...
      const currentPrice = (bonds[bond.ticker].currentPrice =
        bondPrices.securities.data.find(
          priceData =>
  ...
  async calculateSharesPositions(): Promise<void> {
    ...
    const shareTickersString: string = this.shareDeals
      .map(d => d.ticker)
      .join(',');
    const sharePrices: IMoexApiResponseCurrentPrices =
      await this.moexApi.getStocksCurrentPrices(
        MoexMarket.shares,
        shareTickersString,
      );
